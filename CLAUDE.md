...
## La idea
Calendario para gestionar reservas de 4 apartamentos en Madrid sin cruces de fechas

## Para quién
Dueños de los apartamentos (uso propio)

## Enfoque del producto
**Herramienta personal** — yo soy el único usuario. No optimices para "usuarios" hipotéticos: optimiza para MI rutina real (menos pasos para capturar, el dato que YO consulto primero, que abrirla valga la pena en 10 segundos). Si te pido features "por si alguien más la usa algún día", recuérdame en una frase que primero la herramienta me tiene que ganar a MÍ.
...
## Cómo lo hacen hoy
Excel o Google Sheets gestionado manualmente

## Qué duele del proceso
Se cruzan fechas y se hacen doble-reservas sin que el sistema lo impida

## Qué te cuesta (la causa raíz)
Pérdida de dinero y quedar mal con huéspedes cuando hay conflicto de reservas
...
3. Tu app ya NACE con un tema visual: **Serio** (id `seria`),
...
/default-theme/theme-tokens.css` con curl y
   ...
   - Pon `data-theme="seria"` en el `<html>` de `app/layout.tsx`.
   - Descarga `https://raicode.ai
/default-theme/DESIGN.seria.md` con curl y
...
const display = Libre_Franklin({ subsets: ["latin"], weight: ["400", "600", "700"], variable: "--font-display", display: "swap" });
...
```
   ...
   - **Favicon desde el día uno** (el iconito de la pestaña): crea
     ...
     del nombre del proyecto en `Libre Franklin` sobre un
   ...
   - Cuando la app corra por primera vez, dime esta línea tal cual:
     "Tu app arrancó con el tema Serio: azul profundo, grises limpios y tipografía muy legible. Se siente como una herramienta de trabajo. Cámbialo cuando quieras."
...
/wizard/4f164d1f-d61e-41ce-b0ab-b1bb478bcd68/review` que
...
/api/wizard/events \
  ...
  -H "X-Wizard-Token: 352c5986a573f036" \
  -d '{"projectId":"4f164d1f-d61e-41ce-b0ab-b1bb478bcd68","eventName":"build-started"}'
```
...
/api/wizard/events \
  -H "Content-Type: application/json" \
  -H "X-Wizard-Token: 352c5986a573f036" \
  -d '{"projectId":"4f164d1f-d61e-41ce-b0ab-b1bb478bcd68","eventName":"needs-supabase-setup"}'
```
...
/wizard/4f164d1f-d61e-41ce-b0ab-b1bb478bcd68/review.
...
> https://raicode.ai
/wizard/4f164d1f-d61e-41ce-b0ab-b1bb478bcd68/setup-supabase
...
/api/wizard/events \
  -H "Content-Type: application/json" \
  -H "X-Wizard-Token: 352c5986a573f036" \
  -d '{"projectId":"4f164d1f-d61e-41ce-b0ab-b1bb478bcd68","eventName":"needs-vercel-setup"}'
```
...
> dirección en tu navegador** →
> https://raicode.ai
/wizard/4f164d1f-d61e-41ce-b0ab-b1bb478bcd68/setup-vercel
...
/api/wizard/events \
  -H "Content-Type: application/json" \
  -H "X-Wizard-Token: 352c5986a573f036" \
  -d '{"projectId":"4f164d1f-d61e-41ce-b0ab-b1bb478bcd68","eventName":"mvp-ready","payload":{"url":"http://localhost:3000"}}'
```
...
**3. Durante el sub-flow de Diseño + Logo** (`/wizard/4f164d1f-d61e-41ce-b0ab-b1bb478bcd68/setup-design`),
...
/api/wizard/events \
  -H "Content-Type: application/json" \
  -H "X-Wizard-Token: 352c5986a573f036" \
  -d '{"projectId":"4f164d1f-d61e-41ce-b0ab-b1bb478bcd68","eventName":"logo-variants-ready","payload":{"variants":[{"id":"v1","label":"Monograma","svg":"<svg ...>...</svg>"},{"id":"v2","label":"Wordmark","svg":"<svg ...>...</svg>"},{"id":"v3","label":"Mark + wordmark","svg":"<svg ...>...</svg>"}]}}'
```
...
/api/wizard/events \
  -H "Content-Type: application/json" \
  -H "X-Wizard-Token: 352c5986a573f036" \
  -d '{"projectId":"4f164d1f-d61e-41ce-b0ab-b1bb478bcd68","eventName":"design-consultation-done","payload":{"designMd":"<contenido completo de DESIGN.md>","structured":{"palette":[{"name":"primary","hex":"<hex del color principal>","on":"<hex del texto encima>","role":"CTAs, acentos"},{"name":"bg","hex":"<hex del fondo>","on":"<hex del texto>","role":"Fondo de app"}],"typography":[{"role":"display","family":"<fuente de títulos>","weight":700,"sample":"Tu título principal"},{"role":"body","family":"<fuente de texto>","weight":400,"sample":"Texto del contenido"}]}}}'
```
...
/api/wizard/events \
  -H "Content-Type: application/json" \
  -H "X-Wizard-Token: 352c5986a573f036" \
  -d '{"projectId":"4f164d1f-d61e-41ce-b0ab-b1bb478bcd68","eventName":"supabase-setup-complete","payload":{"status":"ok","clientsWritten":2,"migrationsRun":1}}'
```
...
/api/wizard/events \
  -H "Content-Type: application/json" \
  -H "X-Wizard-Token: 352c5986a573f036" \
  -d '{"projectId":"4f164d1f-d61e-41ce-b0ab-b1bb478bcd68","eventName":"vercel-setup-complete","payload":{"status":"ok","productionUrl":"https://<final-url>.vercel.app","envVarsPushed":2}}'
```
...
/api/wizard/events \
  -H "Content-Type: application/json" \
  -H "X-Wizard-Token: 352c5986a573f036" \
  -d '{"projectId":"4f164d1f-d61e-41ce-b0ab-b1bb478bcd68","eventName":"anthropic-setup-complete","payload":{"status":"ok"}}'
```
...
/api/wizard/events \
  -H "Content-Type: application/json" \
  -H "X-Wizard-Token: 352c5986a573f036" \
  -d '{"projectId":"4f164d1f-d61e-41ce-b0ab-b1bb478bcd68","eventName":"gemini-setup-complete","payload":{"status":"ok"}}'
```
